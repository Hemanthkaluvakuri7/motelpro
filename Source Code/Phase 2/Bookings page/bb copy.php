    ...
    <label for="number1" >Number of Rooms</label><br/>
        ...
        <input type="text" name="number1" id="number1" value="1">
         <button type="button" class="counter-plus btn btn-primary"  onclick="pay()">+  </button>
            ...
            <br>
            <input type="date" name="date2" id='d2' value="<?php echo date('Y-m-d'); ?>" onchange="payr()"> 
            <br>
...
document.getElementById("dm3").innerHTML ="Total Cost : "+ String(parseInt(document.getElementById("rooms").value)*(parseInt(document.getElementById("number1").value)-1)*diffDays);
      }
      function paym()
      {var sel = document.getElementById("rooms");
var text= "Cost for Room selected : "+ sel.options[sel.selectedIndex].text;
...
document.getElementById("dm3").innerHTML ="Total Cost : "+ String(parseInt(document.getElementById("rooms").value)*(parseInt(document.getElementById("number1").value)-1));
      }
      function payr()
      {var sel = document.getElementById("rooms");
var text= "Cost for Room selected : "+ sel.options[sel.selectedIndex].text;
...
document.getElementById("dm3").innerHTML ="Total Cost : "+ String(parseInt(document.getElementById("rooms").value)*(parseInt(document.getElementById("number1").value)));
      }
...
if(isset($_POST["ts"]))
{
    if($_POST["ts"])
    {
        $user = 'root';
            ...
            $id=$_SESSION["cid"];
            $nam=$_SESSION["nm"];
            $_SESSION["ro"]='<script>var price = document.getElementById("dm1").value;</script>'; 
            ...
            $SQLstring = "INSERT INTO $tableName VALUES( NULL, '".$id."')";

            $result[] = $mysqli->query($SQLstring);
            if ( $result )

            {
                $URL="http://localhost/conf.php";
